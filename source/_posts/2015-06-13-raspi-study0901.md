...
好，既然动态扫描占用CPU资源太多，那这活儿咱包出去让别人干不就行了？这就是本文主角MAX7219的特长，动态扫描这活儿MAX7219全包了，干的还比咱自己干得好（支持各种设置如亮度设置扫描位数设置等），我们只需要告诉它要显示的内容就行了。
...
- 先看一下MAX7219的引脚图：
![MAX7219的引脚图](01.png)
MAX7219一共有24根引脚，先来看一下跟74HC595的引脚功能类似的引脚。
MAX7219|74HC595|功能
----|----|----
DIN|DS|串行输入
